...
# (frontends/go-sh/ + harness/*) if there are changes, and on
...
while true; do
  ...
  if [ -n "$changes" ]; then
    ...
    echo "[$(date +%FT%T)] go-sh: build start" >> "$LOG"
    if make test >> "$LOG" 2>if make build >> "$LOG" 2>&1; then1; then
      # commit within scope
      git -C "$WORKSPACE" add $changes 2>/dev/null || true
      git -C "$WORKSPACE" commit -m "frontend go-sh: build/fix" >> "$LOG" 2>&1 || true
    else
      # build FAIL: invoke pi (deepseek-v4-flash, scoped)
      echo "[$(date +%FT%T)] go-sh: build FAILED -- invoking pi" >> "$LOG"
