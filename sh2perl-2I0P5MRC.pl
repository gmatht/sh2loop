...
exit 1;
    ...
    $main_exit_code = system('%s\n', "$PROGRAM_NAME: missing operand
Try \\" . chr(96) . "$PROGRAM_NAME --help' for more information.") >> 8;
exit 1;
}
my $tmp;
my @tmp;
...
exit 1;
        ...
        $main_exit_code = system('%s\n', "$PROGRAM_NAME: $i is not a readable regular file") >> 8;
next;
    }
...
exit 1;
                        ...
                        $main_exit_code = system('%s\n', "$PROGRAM_NAME: $i is already gzexe'd") >> 8;
            next;        }
    }
...
exit 1;
        ...
        $main_exit_code = system('%s\n', "$PROGRAM_NAME: $i has setuid permission, unchanged") >> 8;
next;
    }
...
exit 1;
        ...
        $main_exit_code = system('%s\n', "$PROGRAM_NAME: $i has setgid permission, unchanged") >> 8;
next;
    }
...
exit 1;
                ...
                $main_exit_code = system('%s\n', "$PROGRAM_NAME: $i might depend on itself") >> 8;
        next;    }
        my $dir;
    my @dir;
    my %dir;
...
exit 1;
            ...
            $main_exit_code = system('%s\n', "$PROGRAM_NAME: cannot copy $file") >> 8;
next;
    }
...
exit 1;
                ...
                $main_exit_code = system('%s\n', "$PROGRAM_NAME: cannot chmod $tmp") >> 8;
next;
        }
    }
if ((Variable("decomp", false, None) == 0)) {
...
if (-f 'v9') {
my ($in_15);
my $pid_15 = open3($in_15, $out_15, $err_15, 'bash', '-c', q{gzip 'v9'});
close $in_15 or croak 'Close failed: $OS_ERROR';
my $result = do { local $INPUT_RECORD_SEPARATOR = undef; <$out_15> };
close $out_15 or croak 'Close failed: $OS_ERROR';
waitpid $pid_15, 0;
if ( $CHILD_ERROR == 0 ) {
push @results, "Compressed: 'v9'";
} else {
push @results, "Failed to compress: 'v9'";
}
} else {
push @results, "File not found: 'v9'";
}
if (-f "$file") {
my ($in_16);
my $pid_16 = open3($in_16, $out_16, $err_16, 'bash', '-c', 'gzip "$file"');
close $in_16 or croak 'Close failed: $OS_ERROR';
my $result = do { local $INPUT_RECORD_SEPARATOR = undef; <$out_16> };
close $out_16 or croak 'Close failed: $OS_ERROR';
waitpid $pid_16, 0;
if ( $CHILD_ERROR == 0 ) {
push @results, "Compressed: "$file"";
} else {
push @results, "Failed to compress: "$file"";
}
} else {
push @results, "File not found: "$file"";
}
 = join "\n", @results;
...
exit 1;
                ...
                $main_exit_code = system('%s\n', "$PROGRAM_NAME: compression not possible for $i, file unchanged.") >> 8;
next;
        }
}
    else {
...
if (do { local $CHILD_ERROR = 0; my $_pipeline_result = do {
            ...
            {
            local *STDOUT;
            open STDOUT, '>', \$output_18 or die "Cannot redirect STDOUT";
            printf("X\n");
            }
            my @lines = split /\n/msx, $output_18;
            my $num_lines = 1;
        ...
        }
        {
            ...
            my @tail_lines = ();
                        $output_19 = do { my @_qx_cmd = ('tail $tail_n + $skip "$file"'); qx{$_qx_cmd[0]}; };
            ...
            if (-f q{d}) {
            my ($in_22);
            my $pid_22 = open3($in_22, $out_22, $err_22, 'bash', '-c', 'gzip q{d}');
            close $in_22 or croak 'Close failed: $OS_ERROR';
            my $result = do { local $INPUT_RECORD_SEPARATOR = undef; <$out_22> };
...
exit 1;
                ...
                $main_exit_code = system('%s\n', "$PROGRAM_NAME: $i probably not in gzexe format, file unchanged.") >> 8;
next;
        }
    }
...
exit 1;
            ...
            $main_exit_code = system('%s\n', "$PROGRAM_NAME: $tmp: cannot chmod") >> 8;
next;
    }
            do {
local *STDERR;
...
exit 1;
            ...
            $main_exit_code = system('%s\n', "$PROGRAM_NAME: cannot backup $i as $i~") >> 8;
next;
    }
        my $err;
    my $force = 1;
...
exit 1;
            ...
            $main_exit_code = system('%s\n', "$PROGRAM_NAME: cannot rename $tmp to $i") >> 8;
next;
    }
    $tmp = q{};
}
