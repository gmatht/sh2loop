...
while IFS= read -r -d '' file; do
    ...
    # Quick pre-checks to decide whether to run bash -n:
    #   - file ends in .sh, OR
    #   - executable with a short (3-5 char) bare alphanumeric name, OR
    #   - starts with a bash/sh shebang
    bname="${file##*/}"
    case "$bname" in
        *.sh) ;;  # Always test .sh files
        *)
            # Executable with short alphanumeric name?
            if [ -x "$file" ] && echo "$bname" | grep -qE '^[[:alnum:] ._-]{2,6}$'; then
                :  # Test it
            else
                # Check shebang
