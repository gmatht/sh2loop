...
sub create_adduser_conf {
    my $OLDVERSION;
    my @OLDVERSION;
    my %OLDVERSION;
    $OLDVERSION = "$_[0]";
if (!(    $main_exit_code = system('dpkg', '--compare-versions', "$OLDVERSION", 'lt', '3.114') >> 8)) {
...
return q{0};
        ...
        if (!(        $main_exit_code = system('dpkg', '--compare-versions', "$OLDVERSION", 'lt', '3.125') >> 8)) {
            my $SHA;
            my @SHA;
            my %SHA;
            $SHA = "\
...
printf("cannot move unchanged adduser.conf to adduser.conf.update-old. You can continue after answering the dpkg configuration file question.\n");
            ...
            $DIR_MODE = (defined (defined ${DIR_MODE} && ${DIR_MODE} ne q{} ? ${DIR_MODE} : '0750') && (defined ${DIR_MODE} && ${DIR_MODE} ne q{} ? ${DIR_MODE} : '0750') ne q{} ? (defined ${DIR_MODE} && ${DIR_MODE} ne q{} ? ${DIR_MODE} : '0750') : '0750');
if (("$DIR_MODE" ne "0750" && "$DIR_MODE" ne "0755")) {
...
do {
    my $mv_cmd_str = 'mv --no-clobber /etc/adduser.conf /etc/adduser.conf.update-old';
    system $mv_cmd_str;
};
            }
        }
...
if ($arg1 =~ /^upgrade$/msx) {
        create_adduser_conf((defined (defined $_[1] && $_[1] ne q{} ? $_[1] : '') && (defined $_[1] && $_[1] ne q{} ? $_[1] : '') ne q{} ? (defined $_[1] && $_[1] ne q{} ? $_[1] : '') : ''));
}
