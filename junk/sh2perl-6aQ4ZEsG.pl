...
if ( -e "/etc/pm/sleep.d/10_unatteded-upgrades-hibernate" ) {
    ...
    if ((!(    $main_exit_code = system('dpkg', '--compare-versions', "$_[1]", 'lt', "1.8~") >> 8) && (-e $CONFIG.ucftmp))) {
        ...
        if ( -e "$CONFIG.ucftmp" ) {
            ...
            require File::Copy;
            if ( File::Copy::move( "$CONFIG.ucftmp", $dest ) ) {
            } else {
                croak
  "mv: cannot move "$CONFIG.ucftmp" to $dest: $ERRNO\n";
            }
        } else {
            croak "mv: "$CONFIG.ucftmp": No such file or directory\n";
        }
