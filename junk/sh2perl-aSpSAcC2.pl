...
sub possible_dest_module_locations {
    ...
    if (${DEST_MODULE_LOCATION[$count]} ne ${dest_module_location[$count]}) {
                $main_exit_code = system('location[1]', q{=}, $DEST_MODULE_LOCATION[eval { int($count) } // ""]) >> 8;
        $CHILD_ERROR = 0;
    } else {
        $CHILD_ERROR = 1;
    }
...
sub add_source_tree {
    my $from = do {
    my ($in_139, $out_139);
    my $pid_139 = open3($in_139, $out_139, '>&STDERR', 'readlink', '-f', $1);
...
$running_distribution = do {
    my ($in_146, $out_146);
    my $pid_146 = open3($in_146, $out_146, '>&STDERR', 'distro_version');
...
$tmpfile = do {
    my ($in_147, $out_147);
    my $pid_147 = open3($in_147, $out_147, '>&STDERR', 'mktemp_or_die');
...
my $output_ps_fh_8;
{
    ...
    print "Hello, DKMS!\n";
if ($output_148 ne q{} && !$output_printed_148) {
    print $output_148;
}
}
use File::Path qw(make_path);
...
$parallel_jobs = (defined ${parallel_jobs} && ${parallel_jobs} ne q{} ? ${parallel_jobs} : do { my $_result = do {
    my ($in_150, $out_150);
    my $pid_150 = open3($in_150, $out_150, '>&STDERR', 'get_num_cpus');
