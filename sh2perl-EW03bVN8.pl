...
my $USAGE = "[-a] [-r] [-m] [-t] [-n] [-b <newname>] <name>";
my $LONG_USAGE = "git-resurrect attempts to find traces of a branch tip
...
branches.";
my $OPTIONS_KEEPDASHDASH = q{};
my $OPTIONS_STUCKLONG = q{};
my $OPTIONS_SPEC = "\
git resurrect $USAGE
--
...
sub search_reflog {
    my ($file) = @_;
open STDIN, '<', "$ENV{GIT_DIR}" or croak "Cannot read file: $OS_ERROR\n";
...
sub search_reflog_merges {
    my ($file) = @_;
    $main_exit_code = system('git', 'rev-parse',  my ($in_1, $out_1); my $pid_1 = open3($in_1, $out_1, '>&STDERR', 'bash', '-c', 'sed -ne "s~^[^ ]* \\\\([^ ]*\\\\) .*	merge " "$_[0]" ":.*~\\\\1^2~p" /logs/HEAD < "$GIT_DIR"'); close $in_1 or croak 'Close failed: $OS_ERROR'; my $result_1 = do { local $INPUT_RECORD_SEPARATOR = undef; <$out_1> }; close $out_1 or croak 'Close failed: $OS_ERROR'; waitpid $pid_1, 0; $result_1) >> 8;
    return;
}
my $oid_pattern = do {
    do { do {
...
sub search_merges {
    my ($file) = @_;
    # Original bash: git rev-list --all --grep="Merge branch '$_[0]'" \
do {
        my $output_3 = q{};
        my $output_printed_3;
        my $pipeline_success_3 = 1;
                my ($in_4, $out_4);
        my $pid_4 = open3($in_4, $out_4, '>&STDERR', 'git', 'rev-list', '--all', '--grep=Merge branch \'$_[0]\'', '--pretty=tformat:');
...
my $early_exit = q{q};
my $scan_reflog = q{t};
my $scan_reflog_merges = q{};
my $scan_merges = q{};
my $scan_merge_targets = q{};
my $new_name = q{};
...
my $all_strategies = "$scan_reflog$scan_reflog_merges$scan_merges$scan_merge_targets";
...
printf('Most recent: ');
        $main_exit_code = system('git', '--no-pager', 'log', '-1', '--pretty=tformat:', "%h %s", $newest) >> 8;
        say "** $new_name already exists, doing nothing";
    }
}
