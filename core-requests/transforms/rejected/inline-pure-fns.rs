//! inline-pure-fns — v1 RECORD (REJECTED): this is the FIRST offer copy
//! ("updates: none", IrExpr::Array as_ref / BinOpKind::Copy shapes). It
//! swept into the core with two E0599 errors that broke the whole crate
//! build; the fixed v2 (re-offered per the §11 updates protocol) is what
//! the core LANDED (src/transforms/inline_pure_fns.rs) — its canonical
//! workspace copy lives in done/. The v2 header's "v1 in done/" refers to
//! this pre-swap placement; the rejected/ copy is the historical record.
//!
