...
- PROTOCOL-TEXT LEAK (filed, m8 `x := f()`): top-level stmts list
  ...
  output, unparseable downstream). Mechanism (2026-09-12, confirmed
  via echo-trap): Go BOOL helpers (e.g. `isStructType`) ECHO `"true"`/
  `"false"` AND return status. Direct-condition callers (`if
  p.isStructType(...)`) use fnCall-status but the echo leaks to the
  output stream unless captured; multi-line captures (ok + ids) then
  append whole to stmts. m8's leak predates dogfood fixes. m32
  (`x := len(...)`, builtin, no user-func machinery) is clean+green.
  Owner: single convention for bools (echo XOR return, with call-site
  capture discipline) — a transpiler call-convention design decision
  affecting every bool helper.
