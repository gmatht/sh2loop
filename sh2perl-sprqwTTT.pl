...
sub mcarfs_copyin {
    ...
    my @name;
    my %name;
    $name = do { use File::Basename qw(basename); my $basename_output = basename("$_[1]"); $CHILD_ERROR = 0; $basename_output; };
    do {
        local %ENV = %ENV;
        my $TMPDIR = $TMPDIR;
        my $XAR = $XAR;
...
if ("$_[0]" =~ /^list$/msx) {
    ...
    # Builtin command 'shift' not implemented
        mcarfs_copyout("@ARGV");
} elsif ("$_[0]" =~ /^copyin$/msx) {
    # Builtin command 'shift' not implemented
        mcarfs_copyin("@ARGV");
} elsif ("$_[0]" =~ /^rm$/msx) {
    # Builtin command 'shift' not implemented
        mcarfs_rm("@ARGV");
} elsif ("$_[0]" =~ /^mkdir$/msx or "$_[0]" =~ /^rmdir$/msx) {
        do {
local *STDERR;
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
        print "mcarfs: ar archives cannot contain directories.\n";
    };
    exit 1;
} elsif (1) {
        do {
...
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
        do {
    my $__echo_line = "mcarfs: unknown command: \"$_[0]\".";
    print $__echo_line;
    if ( !( $__echo_line =~ m{\n\z}msx ) ) {
        print "\n";
        $__echo_line .= "\n";
