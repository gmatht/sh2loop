    ...
    if ($summary && $summary->{failed} == 0 && !$timed_out && @{$failed_tests} == 0) {
        ...
        if ($csf_summary->{failed} > 0) {
            ...
            my $prompt = join("\n",
                ...
                "    a stuck otranspilerl-cli process is killed after 10 seconds.",
                "  - When REPRODUCING any failing test, ALWAYS run otranspilerl-cli under",
                "    'timeout 15' (e.g. 'timeout 15 ./target/debug/otranspilerl-cli --target pl",
                "    examples/<f>.sh') — some constructs (typeset-cmdsub.sh, assoc",
                "    arrays) hang the perl generator INDEFINITELY at 100% CPU; a",
                "    bounded timeout surfaces the hang without wedging this loop.",
                "",
