...
;
    ...
    my $stderr;
    my $rc;
    $stdout = do { open(my $__fh, '-|', 'bash', '-c', '"${cmd}:@" 2> "/tmp/realpath_stderr.$$"') or die "cmd failed: $!\n"; my $_r = do { local $/; <$__fh> }; close $__fh; chomp $_r; $CHILD_ERROR = $? >> 8; $_r; };
    $rc = $CHILD_ERROR;
    $stderr = do { my $__cs = do {
    my $command = 'cat "/tmp/realpath_stderr.$$" 2> /dev/null || true';
    my ($in, $out, $err);
...
$CHILD_ERROR = 0;
    ...
    $ENV{stdout} = $stdout;
my $output_4 = do { open(my $__fh, '-|', 'bash', '-c', 'printf %s "$stdout" | od -An -t x1z') or die "cmd failed: $!\n"; my $_r = do { local $/; <$__fh> }; close $__fh; chomp $_r; $CHILD_ERROR = $? >> 8; $_r; };
...
my $rc_zero = $CHILD_ERROR;
printf("  (raw output with NULs above; use od to verify):\n");
$CHILD_ERROR = 0;
printf('  ');
$CHILD_ERROR = 0;
# Original bash: realpath --zero /bin /usr/bin/sh | od -A n -t x1z | head -3
my $output_15 = do { open(my $__fh, '-|', 'bash', '-c', 'realpath --zero /bin /usr/bin/sh | od -An -t x1z | head -3') or die "cmd failed: $!\n"; my $_r = do { local $/; <$__fh> }; close $__fh; chomp $_r; $CHILD_ERROR = $? >> 8; $_r; };
if ($output_15 ne q{}) { print $output_15, "\n"; }
printf("  exit code: %d\n\n", "${rc_zero}");
$CHILD_ERROR = 0;
