...
VBIN="$ROOT/.shir-verify-target/debug/otranspilerl-cli"
OTRANSPILERL="$VBIN" SH2_TRANSFORMS=noop "$ROOT/fail-shir" >/dev/null 2>&1
...
for t in targets:
    ...
    a1=subprocess.run([vb,'--target','shir',p],capture_output=True,text=True).stdout
    ...
    perl=subprocess.run([vb,'--source-lang','shir','--target','perl',tmp.name],capture_output=True,text=True).stdout
    os.unlink(tmp.name)
