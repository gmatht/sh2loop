...
open STDERR, '>&', STDOUT or die "Cannot dup stderr: $OS_ERROR\n";
    ...
    $main_exit_code = system('py3clean', '-p', 'python3-dnspython') >> 8;
}
else {
    # Original bash: dpkg -L python3-dnspython | sed -En -e '/^(.*)\/(.+)\.py$/s,,rm "\1/__pycache__/\2".*,e'
do {
        my $output_0 = q{};
        my $output_printed_0;
        my $pipeline_success_0 = 1;
                my ($in_1, $out_1);
        my $pid_1 = open3($in_1, $out_1, '>&STDERR', 'dpkg', '-L', 'python3-dnspython');
...
do {
        ...
        require File::Find;
        my @find_results;
        File::Find::find(sub { if (-d $_ && $_ =~ /^__pycache__$/) { push @find_results, $File::Find::name; } }, '/usr/lib/python3/dist-packages/');
        my $result = join "\n", @find_results;
        ...
        $result;
                ...
                my @xargs_input_2_1 = grep { $_ ne q{} } split /\s+/, $output_2;
        my @xargs_output_2_1;
