...
sub has_option {
if ("$(echo "$OPTIONS" | grep -Eo "^(no-)?$1\>" eq "$1") {
return q{0};
}
    else {
return q{1};
...
if ((`stat -c%s \"$ERRFILE\"` > $MAGIC_500000)) {
    ...
    $T = do {
    my ($in_19, $out_19);
    my $pid_19 = open3($in_19, $out_19, '>&STDERR', 'mktemp', '-p', "$ENV{HOME}");
...
$SESSIONFILES = do {
    my ($in_23, $out_23);
    my $pid_23 = open3($in_23, $out_23, '>&STDERR', 'run-parts', '--list', $SYSSESSIONDIR);
