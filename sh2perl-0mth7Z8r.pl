...
die "Failed to download $url\n";
    ...
    my $REMOTE_VERSION = do { chomp(my $result_3 = qx{grep SCRIPT_VERSION Variable("SHFILE", false, None) | head -n 1 | sed 's/.*=//'}); $result_3; };
    my $OVERWRITE;
...
sub get_alsa_library_version {
    my ($file) = @_;
    my $ALSA_LIB_VERSION = (do { chomp(my $result_36 = qx(grep VERSION_STR /usr/include/alsa/version.h 2> /dev/null | awk '{ print $_[2] }' | sed 's/"//g')); $result_36; });
...
if ("$ENV{DISTRIB_ID}" eq 'Ubuntu') {
      ...
      or die "Close failed: $OS_ERROR\n";
                    ...
                    $ALSA_LIB_VERSION = (do { chomp(my $result_37 = qx(dpkg -l libasound2 | tail -1 | awk '{ print $_[2] }' | cut -f 1 -d -)); $result_37; });
                }
...
if (!(                do {
      ...
      or die "Close failed: $OS_ERROR\n";
                    ...
                    $ALSA_LIB_VERSION = (do { chomp(my $result_38 = qx(dpkg -l libasound2 | tail -1 | awk '{ print $_[2] }' | cut -f 1 -d -)); $result_38; });
                }
if ("$ALSA_LIB_VERSION" eq '<none>') {
                    $ALSA_LIB_VERSION = "";
                }
...
$SYSFS = (do { chomp(my $result_42 = qx(mount | grep sysfs | awk '{ print $3 }')); $result_42; });
$SNDOPTIONS = (do { chomp(my $result_43 = qx{modprobe -c | sed -n "s/^options \\(snd[-_][^ ]*\\)/\\1:/p"}); $result_43; });
my $KEEP_OUTPUT = q{};
$NFILE = "";
$PASTEBIN = "";
...
if ("$KERNEL_VERSION" eq *SMP*) {
    ...
    $ALSA_DRIVER_VERSION = do { chomp(my $result_51 = qx(cat /proc/asound/version | head -n 1 | awk '{ print $7 }' | sed "s/\\.\$//")); $result_51; };
    get_alsa_library_version();
    $ALSA_UTILS_VERSION = do { chomp(my $result_52 = qx(amixer -v | awk '{ print $3 }')); $result_52; };
    $ESDINST = do {
    my ($in_53, $out_53);
...
if ((-d "$SYSFS")) {
      ...
      or die "Close failed: $OS_ERROR\n";
        ...
        my $mod;
        for my $mod (do { chomp(my $result_78 = qx(cat /proc/asound/modules | awk '{ print $2 }')); $result_78; }) {
            do {
                open my $original_stdout, '>&', STDOUT
      or die "Cannot save STDOUT: $OS_ERROR\n";
                open STDOUT, '>>', $FILE
...
my $FINAL_URL;
if ("$PASTEBIN" eq q{}) {
    $FINAL_URL = (do { chomp(my $result_101 = qx{grep SUCCESS: "$TEMPDIR/wget.tmp" | cut -d ' ' -f 2}); $result_101; });
}
else {
    $FINAL_URL = (do { chomp(my $result_102 = qx{grep SUCCESS: "$TEMPDIR/wget.tmp" | sed -n "s/.*\\:\\([0-9]\\+\\).*/https:\\/\\/pastebin.ca\\/\\1/p"}); $result_102; });
}
if ((-x "$TPUT")) {
    $FINAL_URL = (do {
    my ($in_103, $out_103);
