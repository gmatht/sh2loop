...
my %whitelist = map { $_ => 1 } qw(
    exec getVar setVar test pipeline capture captureWords fieldSplit mktemp redirect caseMatch param arith brace setArray setArrayAppend assign arrayItems arrayLen arrayIndex join setLastExit arithEval idiv imod guard not contains builtin grepText cutText bcSqrt fnCall callDirect callUndefined
