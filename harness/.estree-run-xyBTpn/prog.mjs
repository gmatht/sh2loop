...
sh2._init("sh2perl/examples/051_primes.sh", []);
...
let count = 0;
let i = 0;
let is_prime = (...__sh2_args) => sh2.callUndefined("is_prime", __sh2_args);
sh2.assign("y", "+=", "2");
...
sh2.functions.set("is_prime", is_prime = () => { sh2.builtin("local", ["n=$1"]); if (!Number.isNaN(Number(sh2.getVar("n"))) && (Number(sh2.getVar("n")) < Number(2))) { sh2.return("1"); } else { sh2.lastExit = 0; } if (!Number.isNaN(Number(sh2.getVar("n"))) && (Number(sh2.getVar("n")) === Number(2))) { sh2.return("0"); } else { sh2.lastExit = 0; } if (((Number(sh2.getVar("n")) || 0) % 2) === 0) { sh2.return("1"); } else { sh2.lastExit = 0; } sh2.builtin("local", ["sqrt_n=", [sh2.bcSqrt(String(sh2.getVar("n")), 0)]]); (i = 3), (sh2.lastExit = 0), true; sh2.whileLoopSync(() => (!Number.isNaN(Number(i)) && !Number.isNaN(Number(sh2.getVar("sqrt_n")))) && (Number(i) <= Number(sh2.getVar("sqrt_n"))), () => { if (sh2.imod(Number(sh2.getVar("n")) || 0, i) === 0) { sh2.return("1"); } else { sh2.lastExit = 0; } i = i + 2; }); sh2.return("0"); }), true;
process.stdout.write("Finding first 100 prime numbers...\n");
process.stdout.write("This may take a while...\n");
sh2.setArray("primes", ["2"]);
count = 1;
candidate = 3;
while (count < 100) { if (sh2.callDirect(is_prime, [candidate])) { sh2.setArrayAppend("primes", [`${candidate}`]); count = count + 1; if ((count % 10) === 0) { process.stdout.write([String(`Found ${count} primes so far...`)].join(" ") + "\n"); } else { sh2.lastExit = 0; } } else { sh2.lastExit = 0; } candidate = candidate + 2; }
