...
# Original bash: grep -Z -l "pattern" temp_file.txt | tr '\0' '\n'
{
    ...
    if ($expanded_set1_23 =~ /\[:upper:\]/msx) {
    $expanded_set1_23 =~ s/\[:upper:\]/ABCDEFGHIJKLMNOPQRSTUVWXYZ/msx;
    }
    # Handle [:lower:] POSIX class in set1
    if ($expanded_set1_23 =~ /\[:lower:\]/msx) {
    $expanded_set1_23 =~ s/\[:lower:\]/abcdefghijklmnopqrstuvwxyz/msx;
    }
    ...
    if ($expanded_set2_23 =~ /\[:upper:\]/msx) {
    $expanded_set2_23 =~ s/\[:upper:\]/ABCDEFGHIJKLMNOPQRSTUVWXYZ/msx;
    }
    # Handle [:lower:] POSIX class in set2
    if ($expanded_set2_23 =~ /\[:lower:\]/msx) {
    $expanded_set2_23 =~ s/\[:lower:\]/abcdefghijklmnopqrstuvwxyz/msx;
    }
    my $tr_result_22_1 = q{};
