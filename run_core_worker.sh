...
done_requests() {
  ls "$REQS/done"/*.md 2>/dev/null | wc -l
}
...
while true; do
  ...
  if [ "$(a1_gate)" -eq 0 ] && "$ROOT/harness/build-lock.sh" --role core -- cargo test --lib \
    ...
    printf '%s core-worker: GREEN (A1-native %s, %s pending, %s done)\n' "$(date +%FT%T)" "$v" "$(pending_requests | wc -l)" "$(done_requests)" >> "$ROOT/gate-reports/core-worker.report"
    wake_sleepers
  else
    echo "[$(date +%FT%T)] core: gate RED after fixes — left for the next iteration (never bless a regression)" >> "$LOG"
    printf '%s core-worker: RED (A1-native %s, %s pending, %s done)\n' "$(date +%FT%T)" "$v" "$(pending_requests | wc -l)" "$(done_requests)" >> "$ROOT/gate-reports/core-worker.report"
  fi
  sleep "$WATCH"
done
