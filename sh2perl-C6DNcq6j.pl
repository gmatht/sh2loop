...
open STDERR, '>&', STDOUT or die "Cannot dup stderr: $OS_ERROR\n";
    ...
    $main_exit_code = system('py3clean', '-p', 'python3-requests') >> 8;
}
else {
    # Original bash: dpkg -L python3-requests | sed -En -e '/^(.*)\/(.+)\.py$/s,,rm "\1/__pycache__/\2".*,e'
{
        my $output_0 = q{};
        my $output_printed_0;
        my $pipeline_success_0 = 1;
                my ($in_1, $out_1);
        my $pid_1 = open3($in_1, $out_1, '>&STDERR', 'dpkg', '-L', 'python3-requests');
