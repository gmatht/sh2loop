...
sub convert_locale {
    my ($file) = @_;
    # Original bash: echo "$_[0]" | sed -e "s/no_NO/nb_NO/g" -e 's/ks_IN/ks_IN@devanagari/g' -e 's/iw_IL/he_IL/g'
do {
        my $output_0 = q{};
        my $output_printed_0;
...
my $PROVIDED_LOCALES = "aa_DJ.UTF-8 UTF-8
aa_ER UTF-8
aa_ET UTF-8
af_ZA.UTF-8 UTF-8
agr_PE UTF-8
...
if ((-f '/usr/local/share/i18n/SUPPORTED')) {
    $USER_LOCALES = (do { my @_qx_cmd = (q{sed -e '/^[a-zA-Z]/!d' -e 's/ *$//g' /usr/local/share/i18n/SUPPORTED}); chomp(my $result = qx{$_qx_cmd[0]}); $CHILD_ERROR = $? >> 8; $result; });
}
my $GEN_LOCALES;
if ((-e $LG)) {
    $GEN_LOCALES = (do { my @_qx_cmd = (q{sed -e '/^[a-zA-Z]/!d' -e 's/ *$//g' $LG}); chomp(my $result = qx{$_qx_cmd[0]}); $CHILD_ERROR = $? >> 8; $result; });
    $GEN_LOCALES = (do {
    my ($in_1, $out_1);
...
if ((-f $langpack)) {
open STDIN, '<', $langpack or croak "Cannot read file: $OS_ERROR\n";
                my $locale;
                my $charset;
while ( my $L = <> ) {
    chomp $L;
