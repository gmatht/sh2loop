...
do { my $eval_input = $_[0]; system('bash', '-c', $eval_input); $CHILD_ERROR = $? >> 8; };
        $exitval = $?;
    }
;
    if (do {
$CHILD_ERROR = ($main_exit_code = eval { int($exitval > 0) } // "") ? 0 : 1;
        $CHILD_ERROR == 0
                ...
                say "n" . q{ } . "(bad exit status: \$exitval)";
    }
    say "n" . q{ } . "\n";
return $exitval;
    return;
...
$t = do {
    ...
    if ($* eq *-d *) {
                die(ret => 1, "$\"Unable to make temporary directory\"");
        $CHILD_ERROR = 0;
    } else {
        $CHILD_ERROR = 1;
    }
    die(ret => 1, "Unable to make temporary file.");
    return;
}

sub show_usage {
    my ($file) = @_;
    say "$\"Usage: $PROGRAM_NAME [action] [options]\"";
    say "$\"  [action]  = { add | remove | build | install | uninstall | match | autoinstall |\"";
    say "$\"                mktarball | ldtarball | status }\"";
    say "$\"  [options] = [-m module] [-v module-version] [-k kernel-version] [-a arch]\"";
    say "$\"              [-c dkms.conf-location] [-q] [--force] [--force-version-override] [--all]\"";
    say "$\"              [--templatekernel=kernel] [--directive='cli-directive=cli-value']\"";
    say "$\"              [--config=kernel-.config-location] [--archive=tarball-location]\"";
    say "$\"              [--kernelsourcedir=source-location]\"";
    say "$\"              [--binaries-only] [--source-only] [--verbose]\"";
    say "$\"              [--no-depmod] [--modprobe-on-install] [-j number] [--version]\"";
    return;
}

sub VER {
    my ($file) = @_;
    # Original bash: echo $_[0] | sed -e 's:\([^0-9]\)\([0-9]\):\1 \2:g' \
do {
        my $output_3 = q{};
        my $output_printed_3;
...
my @grep_filenames_4 = ();
if (-e "/lib/modules/") {
    open my $fh, '<', "/lib/modules/" or croak "Cannot access file: $ERRNO";
    while (my $line = <$fh>) {
        chomp $line;
...
if (-e "/modules.dep") {
    open my $fh, '<', "/modules.dep" or croak "Cannot access file: $ERRNO";
    while (my $line = <$fh>) {
        chomp $line;
        push @grep_lines_4, $line;
...
$grep_result_4 = q{};
                ...
                my $module_compressed_suffix = ".gz";
    }
    if (do {
                do {
local *STDERR;
...
$grep_result_6 = q{};
    ...
    my $module_suffix = "$module_uncompressed_suffix$module_compressed_suffix";
    return;
}
...
sub check_all_is_banned {
    my ($file) = @_;
if (($all ne q{})) {
        die(ret => 5, "$\"The action $_[0] does not support the --all parameter.\"");
    }
    return;
}
...
if (eval { int(scalar(@kernelver) != 1) } // "") {
        die(ret => 4, "$\"The action $_[0] does not support multiple kernel version parameters on the command line.\"");
    }
    check_all_is_banned($_[0]);
    return;
}
...
if (-e "/proc/cpuinfo") {
    open my $fh, '<', "/proc/cpuinfo" or croak "Cannot access file: $ERRNO";
    while (my $line = <$fh>) {
        chomp $line;
...
do { my $eval_input = "echo" . "$_export_env[$_i]=\\\"${" . $_export_env . "[$_i]}\\\""; system('bash', '-c', $eval_input); $CHILD_ERROR = $? >> 8; };
                }
;
            }
;
    ...
    do { do {
                ...
                say "$directive_name=\"$directive_value\"";
            }
;
        q{};
    };
...
sub read_conf {
    ...
    if (!((-r $read_conf_file))) {
                die(ret => 4, "$\"Could not locate dkms.conf file.\"", "$\"File: $read_conf_file does not exist.\"");
    }
    if (($last_mvka eq $module/$module_version/$_[0]/$_[1] && $last_mvka_conf eq $(readlink -f $read_conf_file))) {
        return;
        $CHILD_ERROR = 0;
    } else {
        $CHILD_ERROR = 1;
...
;
    ...
    for my $directive (@directive_array) {
        ...
        $directive_value = ${directive} =~ s/^.*?=//r;
$ENV{} = '';
        say "$\"DIRECTIVE: $directive_name=\"$directive_value\"\"";
    }
;
    my $clean = "$ENV{CLEAN}";
    my $package_name = "$ENV{PACKAGE_NAME}";
    my $package_version = "$ENV{PACKAGE_VERSION}";
    my $post_add = "$ENV{POST_ADD}";
    my $post_build = "$ENV{POST_BUILD}";
    my $post_install = "$ENV{POST_INSTALL}";
    my $post_remove = "$ENV{POST_REMOVE}";
    my $pre_build = "$ENV{PRE_BUILD}";
    my $pre_install = "$ENV{PRE_INSTALL}";
    my $obsolete_by = "$ENV{OBSOLETE_BY}";
if ((!$package_name)) {
        do {
local *STDERR;
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
            say "$\"dkms.conf: Error! No 'PACKAGE_NAME' directive specified.\"";
        };
        $return_value = q{1};
    }
if ((!$package_version)) {
...
local *STDERR;
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
            say "$\"dkms.conf: Error! No 'PACKAGE_VERSION' directive specified.\"";
        };
        $return_value = q{1};
    }
    my $index;
...
$CHILD_ERROR = ($main_exit_code = eval { int($s > $array_size) } // "") ? 0 : 1;
    ...
    my $strip[$index];
    my $built_module_location[$index];
    my $dest_module_location[$index];
    my $built_module_name[$index];
    my $dest_module_name[$index];
...
local *STDERR;
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
                    say "$\"dkms.conf: Error! No 'BUILT_MODULE_NAME' directive specified for record #$index.\"";
                };
                $return_value = q{1};
            }
...
local *STDERR;
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
                    say "$\"dkms.conf: Error! 'BUILT_MODULE_NAME' directive ends in '.o' or '.ko' in record #$index.\"";
                };
                                $return_value = q{1};
            }
if (q{} =~ /^.*.o$/msx or q{} =~ /^.*.ko$/msx) {
...
local *STDERR;
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
                    say "$\"dkms.conf: Error! 'DEST_MODULE_NAME' directive ends in '.o' or '.ko' in record #$index.\"";
                };
                                $return_value = q{1};
            }
            $dest_module_location{"$index"} = (do {
    my ($in_29, $out_29);
...
local *STDERR;
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
                    say "$\"dkms.conf: Error! No 'DEST_MODULE_LOCATION' directive specified for record #$index.\"";
                };
                $return_value = q{1};
            }
if (q{} =~ /^/kernel.*$/msx) {
...
local *STDERR;
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
                    say "$\"dkms.conf: Error! Directive 'DEST_MODULE_LOCATION' does not begin with\"";
                };
                                do {
local *STDERR;
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
                    say "$\"'/kernel', '/updates', or '/extra' in record #$index.\"";
                };
                                $return_value = q{1};
            }
    }
...
local *STDERR;
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
            say "$\"dkms.conf: Warning! Zero modules specified.\"";
        };
    }
    if (!((${MAKE_MATCH[0]} ne q{}))) {
                my $make_command = $MAKE[0];
    }
...
if (-e "y") {
    open my $fh, '<', "y" or croak "Cannot access file: $ERRNO";
    while (my $line = <$fh>) {
        chomp $line;
        push @grep_lines_34, $line;
...
;
    if ((($BUILD_EXCLUSIVE_KERNEL ne q{}) && !$_[0] =~ /$BUILD_EXCLUSIVE_KERNEL/msx)) {
                my $build_exclude = "yes";
        $CHILD_ERROR = 0;
    } else {
        $CHILD_ERROR = 1;
    }
    if ((($BUILD_EXCLUSIVE_KERNEL_MIN ne q{}) && (qx'VER "$_[0]"'"<"qx'VER "$BUILD_EXCLUSIVE_KERNEL_MIN"' ne q{}))) {
                $build_exclude = "yes";
        $CHILD_ERROR = 0;
    } else {
        $CHILD_ERROR = 1;
    }
    if ((($BUILD_EXCLUSIVE_KERNEL_MAX ne q{}) && (qx'VER "$_[0]"'">"qx'VER "$BUILD_EXCLUSIVE_KERNEL_MAX"' ne q{}))) {
                $build_exclude = "yes";
        $CHILD_ERROR = 0;
    } else {
        $CHILD_ERROR = 1;
    }
    if ((($BUILD_EXCLUSIVE_ARCH ne q{}) && !$_[1] =~ /$BUILD_EXCLUSIVE_ARCH/msx)) {
                $build_exclude = "yes";
        $CHILD_ERROR = 0;
    } else {
        $CHILD_ERROR = 1;
...
$CHILD_ERROR = ($main_exit_code = eval { int($return_value == 0) } // "") ? 0 : 1;
    $CHILD_ERROR == 0
        ...
        my $last_mvka = "$module/$ENV{module_version}/$_[0]/$_[1]";
}
        $CHILD_ERROR == 0
    }) {
                my $last_mvka_conf = (do {
    my ($in_38, $out_38);
...
;
    say '-E' . q{ } . ${ver};
    say '-E' . q{ } . (defined ${srcver} && ${srcver} ne q{} ? ${srcver} : '$checksum');
    return;
}
...
sub check_version_sanity {
    ...
    my $res = q{};
    say "$\"Running module version sanity check.\"";
    my $i = "0";
    my $obsolete;
if ("$_[2]" ne q{}) {
        my @obs = ('${3//-/', '}');
...
local *STDERR;
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
                say "$\"Module has been obsoleted due to being included\"";
            };
            do {
local *STDERR;
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
                say "$\"in kernel $_[2].  We will avoid installing\"";
            };
            do {
local *STDERR;
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
                say "$\"for future kernels above $_[2].\"";
            };
            do {
local *STDERR;
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
                say "$\"You may override by specifying --force.\"";
            };
return q{1};
        }
    }
...
local *STDERR;
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
                        say "$\"Module version $(echo \"" . q{ } . $verinfo . q{ } . " | head -n 1) for $_[3]" . ($ENV{module_suffix} // q{});
                    };
                    do {
local *STDERR;
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
                        say "$\"exactly matches what is already found in kernel $_[0].\"";
                    };
                    do {
local *STDERR;
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
                        say "$\"DKMS will not replace this module.\"";
                    };
                    do {
local *STDERR;
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
...
sub check_module_args {
    ...
    die(ret => 1, "$\"Arguments <module> and <module-version> are not specified.\"", "$\"Usage: $_[0] <module>/<module-version> or\"", "$\"       $_[0] -m <module>/<module-version> or\"", "$\"       $_[0] -m <module> -v <module-version>\"");
    return;
}
...
read_conf("\@ARGV");
    ...
    die(ret => 8, "$\"Bad conf file.\"", "$\"File: " . (defined (defined $_[2] && $_[2] ne q{} ? $_[2] : '$conf') && (defined $_[2] && $_[2] ne q{} ? $_[2] : '$conf') ne q{} ? (defined $_[2] && $_[2] ne q{} ? $_[2] : '$conf') : '$conf') . " does not represent a valid dkms.conf file.\"");
    return;
}
...
if (($rpm_safe_upgrade ne q{})) {
        my $pppid = do { my @_qx_cmd = (q(awk '/PPid:/ {print $_[1]}' /proc/ $PPID /status)); chomp(my $result = qx{$_qx_cmd[0]}); $CHILD_ERROR = $? >> 8; $result; };
        my $lock_name = do {
    my ($in_46, $out_46);
...
if (!(system('is_module_added', "$module", "$ENV{module_version}") >> 8)) {
        die(ret => 3, "$\"DKMS tree already contains: $module-$ENV{module_version}\"", "$\"You cannot add the same module/version combo more than once.\"");
    }
    if (!(($conf ne q{}))) {
                my $conf = "$ENV{source_tree}/$module-$ENV{module_version}/dkms.conf";
    }
if ((-d $source_tree/$module-$module_version)) {
        die(ret => 2, "$\"Could not find module source directory.\"", "$\"Directory: $ENV{source_tree}/$module-$ENV{module_version} does not exist.\"");
    }
    read_conf_or_die("$kernelver", "$arch", "$conf");
    say "$\"Creating symlink $ENV{dkms_tree}/$module/$ENV{module_version}/source -> $ENV{source_tree}/$module-$ENV{module_version}\"";
    use File::Path qw(make_path);
    my $err;
...
sub prepare_kernel {
    ...
    if ($CHILD_ERROR != 0) {
                    die(ret => 1, "$\"Your kernel headers for kernel $_[0] cannot be found at /lib/modules/$_[0]/build or /lib/modules/$_[0]/source.\"", "$\"Please install the linux-headers-$_[0] package or use the --kernelsourcedir option to tell DKMS where it's located.\"");
    }
;
    return;
}
...
sub prepare_signing {
    my $do_signing = q{0};
if ((!-f "${kernel_config}")) {
        say "Kernel config " . ($ENV{kernel_config} // q{}) . " not found, modules won't be signed";
return;
    }
...
$grep_result_49 = join "\n", @grep_filtered_49;
    if (!($grep_result_49 =~ m{\n\z} || $grep_result_49 eq q{})) {
        $grep_result_49 .= "\n";
    }
$CHILD_ERROR = scalar @grep_filtered_49 > 0 ? 0 : 1;
$grep_result_49 = q{}) {
        say "The kernel is be built without module signing facility, modules won't be signed";
return;
    }
    my $sign_hash = do {
    do { my $output_50 = q{};
...
;
    say "Sign command: $sign_file";
if (((!-f "${sign_file}") || (!-x "${sign_file}"))) {
        say "Binary " . ${sign_file} . " not found, modules won't be signed";
return;
    }
    my $mok_certificate;
    my $mok_signing_key;
...
if ((!-x "qx'command -v update-secureboot-policy'" ne q{})) {
                    say "Binary update-secureboot-policy not found, modules won't be signed";
return;
                }
...
if ( -e "${mok_certificate}" ) {
          ...
          " is a directory (use -r to remove recursively)\n";
              ...
              ": $OS_ERROR\n";
                ...
                say "Certificate or key are missing, generating them using update-secureboot-policy...";
...
if ((!"${mok_signing_key}")) {
    ...
    say "Signing key: $mok_signing_key";
if ((!"${mok_certificate}")) {
        $mok_certificate = "/var/lib/dkms/mok.pub";
    }
    say "Public certificate (MOK): $mok_certificate";
if (("$mok_signing_key" ne "pkcs11:"* && !(    do {
        ...
        say "Certificate or key are missing, generating self signed certificate for MOK...";
if (        do {
            open my $original_stdout, '>&', STDOUT
      or die "Cannot save STDOUT: $OS_ERROR\n";
            open STDOUT, '>', '/dev/null'
      ...
      or die "Close failed: $OS_ERROR\n";
            ...
            say "openssl not found, can't generate key and certificate.";
return;
        }
        do {
            open my $original_stdout, '>&', STDOUT
...
if ((!-f "${mok_signing_key}")) {
            say "Key file " . ${mok_signing_key} . " not found and can't be generated, modules won't be signed";
return;
        }
    }
if ((!-f "${mok_certificate}")) {
        say "Certificate file " . ${mok_certificate} . " not found and can't be generated, modules won't be signed";
return;
    }
    $do_signing = q{1};
    return;
...
;
    ...
    if ((-d $base_dir)) {
                die(ret => 3, "$\"This module/version has already been built on: $kernelver\"", "$\"Directory $base_dir already exists. Use the dkms remove function before trying to build again.\"");
        $CHILD_ERROR = 0;
    } else {
        $CHILD_ERROR = 1;
    }
    ...
    if (($build_exclude ne q{})) {
                die(ret => 77, "$\"The $base_dir/dkms.conf for module $module includes a BUILD_EXCLUSIVE directive which does not match this kernel/arch/config.\"", "$\"This indicates that it should not be built.\"");
        $CHILD_ERROR = 0;
    } else {
        $CHILD_ERROR = 1;
    }
    if (do {
$CHILD_ERROR = ($main_exit_code = eval { int(do { chomp(my $_r = qx'ls $source_dir | wc -l | awk {'\''print $_[0]'\''}'); $_r; } < 2) } // "") ? 0 : 1;
        $CHILD_ERROR == 0
    }) {
                die(ret => 8, "$\"The directory $source_dir does not appear to have module source located within it.\"", "$\"Build halted.\"");
    }
if ( -e "$build_dir" ) {
        if ( -d "$build_dir" ) {
            my $err;
...
delete $ENV{CC};
    ...
    do {
      ...
      or die "Cannot access file: $OS_ERROR\n";
        my $tmp = do {
        say "$\"DKMS make.log for $module-$ENV{module_version} for kernel $kernelver ($arch)\"";
        };
        print $tmp;
        open STDOUT, '>&', $original_stdout
      or die "Cannot restore STDOUT: $OS_ERROR\n";
...
;
    for ($ENV{count}=0) ; $ENV{count} < 0) ; $ENV{count}++) ) {
...
if (eval { int($ENV{do_signing}) } // "") {
                say "Signing module $built_module";
                $CHILD_ERROR = 0;
            }
...
sub force_installation {
    my $forced_modules_dir = "/usr/share/dkms/modules_to_force_install";
    my $to_force = "";
if ((-d $forced_modules_dir)) {
        my $elem;
        for my $elem ($forced_modules_dir, '/*') {
if ((-e $elem)) {
                $to_force = "$to_force " . (do { my $cat_chunk = q{}; if ( open my $fh, '<', $elem ) { local $INPUT_RECORD_SEPARATOR = undef; $cat_chunk = <$fh>; close $fh; } else { carp 'cat: ' . $elem . ': ' . $OS_ERROR . "\n"; } $cat_chunk; });
            }
        }
;
...
if ("$ret" =~ /"force"/msx) {
        $force = "true";
        say "Forcing installation of $module";
}
    else {
        if ("$ret" =~ /"version-override"/msx) {
            $force_version_override = "true";
            say "Forcing version override of $module";
        }
    }
;
    if (!((-e $install_tree/$kernelver))) {
                die(ret => 6, "$\"The directory $ENV{install_tree}/$kernelver doesn't exist.\"", "$\"You cannot install a module onto a non-existant kernel.\"");
    }
    read_conf_or_die("$kernelver", "$arch");
    if (do {
$main_exit_code = system('is_module_installed', "$module", "$ENV{module_version}", "$kernelver", "$arch") >> 8;
        $CHILD_ERROR == 0
                ...
                die(ret => 5, "$\"This module/version combo is already installed for kernel $kernelver ($arch).\"");
    }
    if (($rpm_safe_upgrade ne q{})) {
                $force = "true";
        $CHILD_ERROR = 0;
    ...
    for ($count=0) ; $count < 0) ; $count++) ) {
            say "$\"\"";
            say "$\"" . $dest_module_name[eval { int($count) } // ""] . "$ENV{module_suffix}:\"";
...
if (((!(            $CHILD_ERROR = ($main_exit_code = eval { int($count == 0) } // "") ? 0 : 1) && !(!(run_build_script('pre_install', "$ENV{pre_install}");))) && !(!(($force ne q{}))))) {
                die(ret => 101, "$\"pre_install failed, aborting install.\"", "$\"You may override by specifying --force.\"");
            }
...
if (((-l $dkms_tree/$module/kernel-$kernelver-$arch) && "$original_copy" ne q{})) {
                say "$\"   - An original module was already stored during a previous install\"";
}
            else {
...
if ("$ENV{running_distribution}" =~ /^debian.*$/msx or "$ENV{running_distribution}" =~ /^ubuntu.*$/msx) {
                                                        ...
                                                        say "$\"   - Found $original_module\"";
                                                        say "$\"   - Storing in $ENV{dkms_tree}/$module/original_module/$kernelver/$arch/\"";
                                                        say "$\"   - Archiving for uninstallation purposes\"";
                                                        use File::Path qw(make_path);
                            my $err;
...
if (((!$found_original) && !(                    $CHILD_ERROR = ($main_exit_code = eval { int($module_count > 1) } // "") ? 0 : 1))) {
                        say "$\"   - Multiple original modules exist but DKMS does not know which to pick\"";
                        say "$\"   - Due to the confusion, none will be considered during a later uninstall\"";
}
                    else {
                        if ((!$found_original)) {
                            say "$\"   - No original module exists within this kernel\"";
                        }
                    }
}
                else {
                    say "$\"   - This kernel never originally had a module by this name\"";
                }
            }
if (eval { int($module_count > 1) } // "") {
                say "$\" - Multiple same named modules!\"";
                say "$\"   - $module_count named $m$ENV{module_suffix} in $lib_tree/\"";
if ("$ENV{running_distribution}" =~ /^debian.*$/msx or "$ENV{running_distribution}" =~ /^ubuntu.*$/msx) {
                } elsif (1) {
                                        say "$\"   - All instances of this module will now be stored for reference purposes ONLY\"";
                                        say "$\"   - Storing in $ENV{dkms_tree}/$module/original_module/$kernelver/$arch/collisions/\"";
                }
                my $module_dup;
...
;
            ...
            say "$\" - Installation\"";
            say "$\"   - Installing to $ENV{install_tree}/$kernelver" . $dest_module_location[eval { int($count) } // ""] . "/\"";
            use File::Path qw(make_path);
            if ( !-d $install_tree ) {
...
if ((!(    $CHILD_ERROR = ($main_exit_code = eval { int(0 > 0) } // "") ? 0 : 1) && (!"${any_module_installed}"))) {
        die(ret => 6, "$\"Installation aborted.\"");
    }
    do {
local *STDERR;
open STDERR, '>', '/dev/null' or croak "Cannot access file: $OS_ERROR\n";
...
;
    ...
    if ($CHILD_ERROR != 0) {
            ...
            die(ret => 6, "$\"Problems with depmod detected. Automatically uninstalling this module.\"", "$\"Install Failed (depmod problems). Module rolled back to built state.\"");
exit 6;
    }
;
...
do {
            ...
            require File::Find;
            my @find_results;
            File::Find::find(sub { if ($_ =~ /^modalias$/) { push @find_results, $File::Find::name; } }, '/sys/devices');
            my $result = join "\n", @find_results;
            ...
            $result;
                        ...
                        my @xargs_input_84_1 = grep { $_ ne q{} } split /\s+/, $output_84;
            ...
            $output_84 = $output_84_2;
                        ...
                        my @xargs_input_84_3 = grep { $_ ne q{} } split /\s+/, $output_84;
...
sub list_each_installed_module {
    ...
    my $mod;
    for ($count=0) ; $count < 0) ; $count++) ) {
            $real_dest_module_location = (do {
    my ($in_85, $out_85);
    my $pid_85 = open3($in_85, $out_85, '>&STDERR', 'find_actual_dest_module_location', $_[0], $count, $_[1], $_[2]);
...
sub do_uninstall {
    my ($file) = @_;
    say "$\"Module $module-$ENV{module_version} for kernel $_[0] ($_[1]).\"";
    set_module_suffix("$_[0]");
    my $was_active = "";
    ...
    my $real_dest_module_location;
    my $dir_to_remove;
if ($kernel_symlink eq $dkms_tree/$module/$module_version/$_[0]/$_[1]) {
        $was_active = "true";
        say "$\"Before uninstall, this module version was ACTIVE on this kernel.\"";
if ("$NO_WEAK_MODULES" eq q{}) {
...
do {
                ...
                say "$\"Removing any linked weak-modules\"";
                # Original bash: list_each_installed_module "$module" "$_[0]" "$_[1]" | ${weak_modules} ${weak_modules_no_initrd} --remove-modules
do {
                    my $output_94 = q{};
...
;
        ...
        for ($ENV{count}=0) ; $ENV{count} < 0) ; $ENV{count}++) ) {
                $real_dest_module_location = (do {
    my ($in_98, $out_98);
    my $pid_98 = open3($in_98, $out_98, '>&STDERR', 'find_actual_dest_module_location', $module, $count, $_[0], $_[1]);
...
if (("${real_dest_module_location}")) {
                    say "$\"   - Deleting from: $ENV{install_tree}/$_[0]" . ${real_dest_module_location} . "/\"";
...
;
                    ...
                    say "$\"   - Module was not found within $ENV{install_tree}/$_[0]/\"";
                }
                say "$\" - Original module\"";
                my $origmod = do {
    my ($in_103, $out_103);
...
if ("$ENV{running_distribution}" =~ /^debian.*$/msx or "$ENV{running_distribution}" =~ /^ubuntu.*$/msx) {
                                                ...
                                                say "$\"   - Archived original module found in the DKMS tree\"";
                                                say "$\"   - Moving it to: $ENV{install_tree}/$_[0]" . $DEST_MODULE_LOCATION[eval { int($count) } // ""] . "/\"";
                                                use File::Path qw(make_path);
                        my $err;
...
open STDERR, '>', '/dev/null' or croak "Cannot access file: $OS_ERROR\n";
  ...
  "mv: cannot move "$origmod" to $dest: $ERRNO\n";
                    ...
                    say "$\"   - No original module was found for this module on this kernel.\"";
                    say "$\"   - Use the dkms install command to reinstall any previous module version.\"";
                }
        }
if ( -e "$ENV{dkms_tree}/$module/kernel-$_[0]-$_[1]" ) {
        ...
        say "$\"This module version was INACTIVE for this kernel.\"";
    }
;
...
if (0) {
        say "$\"\"";
        say "$\"Removing original_module from DKMS tree for kernel $_[0] ($_[1])\"";
        do {
local *STDERR;
open STDERR, '>', '/dev/null' or croak "Cannot access file: $OS_ERROR\n";
if ( -e "$ENV{dkms_tree}/$module/original_module/$_[0]/$_[1]" ) {
        ...
        if (!((qx'find $dkms_tree/$module/original_module/$_[0]/* -maxdepth 0 -type d 2>/dev/null' ne q{}))) {
        ...
        if ((($was_active ne q{}) && (-d $dkms_tree/$module/original_module/$_[0]/$_[1]/collisions))) {
            say "$\"\"";
            say "$\"Keeping directory $ENV{dkms_tree}/$module/original_module/$_[0]/$_[1]/collisions/\"";
            say "$\"for your reference purposes.  Your kernel originally contained multiple\"";
            say "$\"same-named modules and this directory is now where these are located.\"";
        }
    }
...
sub module_is_added_or_die {
    ...
    if ($CHILD_ERROR != 0) {
                die(ret => 3, "$\"The module/version combo: $module-$ENV{module_version} is not located in the DKMS tree.\"");
    }
;
    return;
}
...
sub maybe_unbuild_module {
    ...
    if ($CHILD_ERROR != 0) {
                    say "$\"Module $module $ENV{module_version} is not built for kernel $_[0] ($_[1]).\"" . q{ } . "$\"Skipping...\"";
return q{0};
    }
;
...
sub maybe_uninstall_module {
    ...
    if ($CHILD_ERROR != 0) {
                    say "$\"Module $module $ENV{module_version} is not installed for kernel $_[0] ($_[1]).\"" . q{ } . "$\"Skipping...\"";
return q{0};
    }
;
...
if ( -e "$ENV{dkms_tree}/$module/$ENV{module_version}/$_[0]/$_[1]" ) {
    ...
    if (!((qx'find $dkms_tree/$module/$module_version/$_[0]/* -maxdepth 0 -type d 2>/dev/null' ne q{}))) {
...
if ( -e "$lock_file" ) {
          ...
          " is a directory (use -r to remove recursively)\n";
            ...
            die(ret => 0, "$\"Remove cancelled because --rpm_safe_upgrade scenario detected.\"");
        }
;
    }
;
...
$tmp_redirect_107;
        ...
        say "$\"Deleting module $module-$ENV{module_version} completely from the DKMS tree.\"";
...
if (eval { int(do { chomp(my $_r = qx'ls "$dkms_tree/$module" | wc -w | awk '\''{print $_[0]}'\'''); $_r; } == 0) } // "") {
        do {
local *STDERR;
open STDERR, '>', '/dev/null' or croak "Cannot access file: $OS_ERROR\n";
...
chomp $m;
$CHILD_ERROR = defined($m) ? 0 : 1;
        say "$m, $v, $k, $a: installed-weak from $kern";
    }
;
    return;
}
...
sub module_status_built {
    ...
    for my $directory ("$ENV{dkms_tree}/$_[0]/$_[1]/", (defined $_[2] && $_[2] ne q{} ? $_[2] : '+([0-9]).*'), q{/}, (defined $_[3] && $_[3] ne q{} ? $_[3] : '*')) {
        $IFS = "$oifs";
        $ka = (${directory} =~ s/^\$dkms_tree/\$_[0]/\$_[1]///r =~ s/^\$dkms_tree/\$_[0]/\$_[1]///r);
        $k = ( ( dirname(${ka}) ) =~ s|/[^/]*$||sr );
        $a = (${ka} =~ s/^.*?///r =~ s/^.*?///r);
...
open STDIN, '<', "$tmpfile" or croak "Cannot read file: $OS_ERROR\n";
while ( my $L = <> ) {
    chomp $L;
    my @_fields = split /\s+/msx, $L;
    $status = $_fields[0] // q{};
...
if ((-d "$intree_module_dir")) {
                    die(ret => 6, "$\"No modules built for " . $kernelver[eval { int($i) } // ""] . " (" . $arch[eval { int($i) } // ""] . ").\"", "$\"Modules must already be in the built state before using mktarball.\"");
                }
                set_module_suffix($kernelver[eval { int($i) } // ""]);
                say "Marking modules for " . $kernelver[eval { int($i) } // ""] . " (" . $arch[eval { int($i) } // ""] . ") for archiving...";
...
if (($binaries_only ne q{})) {
        ...
        say "$\"\"";
        say "$\"Creating tarball structure to specifically accomodate binaries.\"";
        use File::Path qw(make_path);
...
open STDERR, '>', '/dev/null' or croak "Cannot access file: $OS_ERROR\n";
        ...
        say "$\"\"";
        say "$\"Marking $source_dir for archiving...\"";
        use File::Path qw(make_path);
        if ( !-d $temp_dir_name ) {
            make_path( $temp_dir_name, { error => \$err } );
            if ( @{$err} ) {
...
if (${archive_location%/*} ne $archive_location) {
    ...
    say "$\"\"";
    say "$\"Tarball location: $tarball_dest/$tarball_name\"";
if ((!-d $tarball_dest)) {
if (        do {
local *STDERR;
open STDERR, '>', '/dev/null' or croak "Cannot access file: $OS_ERROR\n";
            ...
            die(ret => 9, "$\"Missing write permissions for $tarball_dest.\"");
        }
    }
    if (!((-w $tarball_dest))) {
                die(ret => 9, "$\"Missing write permissions for $tarball_dest.\"");
...
open STDERR, '>', '/dev/null' or croak "Cannot access file: $OS_ERROR\n";
        ...
        die(ret => 6, "$\"Failed to make tarball.\"");
    }
    return;
}
...
if ((!-e $archive_location)) {
        die(ret => 2, "$\"$ENV{archive_location} does not exist.\"");
    }
if ($archive_location eq *.rpm) {
if (!(system('rpm', '-Uvh', "$ENV{archive_location}") >> 8)) {
            $main_exit_code = system('bash', 'autoinstall') >> 8;
}
        else {
            die(ret => 9, "$\"Unable to install $ENV{archive_location} using rpm.\"", "$\"Check to ensure that your " . "sys" . "tem" . " can install .rpm files.\"");
        }
    }
    my $temp_dir_name = do {
    my ($in_126, $out_126);
...
if ((!-d $temp_dir_name/dkms_main_tree)) {
    ...
    do { do {
            ...
            require File::Find;
            my @find_results;
            File::Find::find(sub { if ($_ =~ /^dkms\.conf$/) { push @find_results, $File::Find::name; } }, q{/});
            my $result = join "\n", @find_results;
            if ($result ne q{}) { $result .= "\n"; }
            $CHILD_ERROR = 0;
            $result;
...
if ((!$conf)) {
            die(ret => 3, "$\"Tarball does not appear to be a correctly formed DKMS archive. No dkms.conf found within it.\"");
        }
...
if ((!$loc)) {
            die(ret => 7, "$\"No valid dkms.conf in dkms_source_tree or dkms_binaries_only.\"", "$\"$ENV{archive_location} is not a valid DKMS tarball.\"");
        }
...
local *STDERR;
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
                say "$\"Malformed dkms.conf, refusing to load.\"";
            };
next;
        }
if ((!(        is_module_added("$ENV{PACKAGE_NAME}", "$ENV{PACKAGE_VERSION}")) && (!$force))) {
            die(ret => 8, "$\"$ENV{PACKAGE_NAME}-$ENV{PACKAGE_VERSION} is already added!\"", "$\"Aborting.\"");
        }
last;
    }
;
    $module = "$ENV{PACKAGE_NAME}";
    my $module_version = "$ENV{PACKAGE_VERSION}";
    say "$\"\"";
    say "$\"Loading tarball for $module-$module_version\"";
...
;
            say "$\"Copying dkms.conf to $source_dir ...\"";
            use File::Copy qw(copy);
            if (-d "$source_dir") {
...
sub run_match {
    set_kernel_source_dir_and_kconfig("$kernelver");
if ((!$template_kernel)) {
        die(ret => 1, "$\"Invalid number of parameters passed.\"", "$\"Usage: match --templatekernel=<kernel-version> -k <kernel-version>\"", "$\"   or: match --templatekernel=<kernel-version> -k <kernel-version> <module>\"");
    }
if ($template_kernel eq $kernelver) {
        die(ret => 2, "$\"The templatekernel and the specified kernel version are the same.\"");
    }
    my $template_kernel_status = do {
    do { do {
        my $output_132 = q{};
...
if ((-d $dkms_tree/$module/)) {
            die(ret => 3, "$\"The module: $module is not located in the DKMS tree.\"");
        }
        $template_kernel_status = do {
    do { do {
            my $output_134 = q{};
    ...
    say "$\"\"";
    say "$\"Matching modules in kernel: $kernelver ($arch)\"";
    say "$\"to the configuration of kernel: $ENV{template_kernel} ($arch)\"";
if ((!$template_kernel_status)) {
        say "$\"\"";
        say "$\"There is nothing to be done for this match.\"";
return q{0};
    }
    prepare_kernel("$kernelver", "$arch");
...
while ( my $L = <> ) {
    ...
    do { do {
        ...
        say "$\"Module:  $template_module\"";
        say "$\"Version: $template_version\"";
...
sub check_root {
    ...
    die(ret => 1, "$\"You must be root to use this command.\"");
    return;
}
...
sub check_rw_dkms_tree {
    ...
    die(ret => 1, "$\"No write access to DKMS tree at " . ($ENV{dkms_tree} // q{}) . "\"");
    return;
}
...
sub add_source_tree {
    ...
    my $from = do {
    my ($in_141, $out_141);
    my $pid_141 = open3($in_141, $out_141, '>&STDERR', 'readlink', '-f', $_[0]);
...
if ((($from ne q{}) && (-f $from/dkms.conf))) {
        die(ret => 9, "$\"$_[0] must contain a dkms.conf file!\"");
    }
    check_root();
    setup_kernels_arches();
if (    get_pkginfo_from_conf("$from/dkms.conf")) {
        die(ret => 10, "$\"Malformed dkms.conf file. Cannot load source tree.\"");
    }
    $module = "$ENV{PACKAGE_NAME}";
    my $module_version = "$ENV{PACKAGE_VERSION}";
...
open STDERR, '>&', STDERR or die "Cannot dup stderr: $OS_ERROR\n";
        ...
        say "$\"Forcing install of $module-$module_version\"";
...
if ("${build_depends[$m]}" eq q{}) {
                do {
                    local %ENV = %ENV;
                    my $known_modules[${#known_modules[@]}] = $known_modules[${#known_modules[@]}];
                    my $to_install[${#to_install[@]}] = $to_install[${#to_install[@]}];
                    my $known_modules = $known_modules;
                    my $installed_modules[${#installed_modules[@]}] = $installed_modules[${#installed_modules[@]}];
                    my $failed_modules[${#failed_modules[@]}] = $failed_modules[${#failed_modules[@]}];
                    my $m = $m;
                    my $next_depends = $next_depends;
                    my $build_depends[$m] = $build_depends[$m];
                    my $installed_modules = $installed_modules;
                    my $status = $status;
                    my $mv = $mv;
                    my $to_install = $to_install;
                    my $next_install = $next_install;
                    my $skipped_modules[${#skipped_modules[@]}] = $skipped_modules[${#skipped_modules[@]}];
                    my $latest["$m"] = $latest["$m"];
                    my $k = $k;
                    my $i = $i;
                    my $build_depends["$m"] = $build_depends["$m"];
                    my $skipped_modules = $skipped_modules;
                    ...
                    my $d = $d;
                    my $failed_modules = $failed_modules;
                    my $next_install[${#next_install[@]}] = $next_install[${#next_install[@]}];
                    my $v = $v;
...
if ((${#installed_modules[@]} > 0)) {
        say "dkms autoinstall on $kernelver/$arch succeeded for " . (join(" ", @installed_modules));
    }
if ((${#skipped_modules[@]} > 0)) {
        say "dkms autoinstall on $kernelver/$arch was skipped for " . (join(" ", @skipped_modules));
    }
if ((${#failed_modules[@]} > 0)) {
        say "dkms autoinstall on $kernelver/$arch failed for " . (join(" ", @failed_modules));
    }
    for my $mv (@to_install) {
            $IFS = q{/};
$m = <>;
chomp $m;
$CHILD_ERROR = defined($m) ? 0 : 1;
        say "$m/$v autoinstall failed due to missing dependencies: " . $build_depends{$m};
    }
if (((${#failed_modules[@]} > 0) || (${#to_install[@]} > 0))) {
        die(ret => 11, "$\"One or more modules failed to install during autoinstall.\"", "$\"Refer to previous errors for more information.\"");
    }
    }
    }
    }
...
my $PATH = "$ENV{PATH}:/usr/lib/dkms";
$main_exit_code = system('umask', q{022}) >> 8;
delete $ENV{CC};
delete $ENV{CXX};
delete $ENV{CFLAGS};
delete $ENV{CXXFLAGS};
delete $ENV{LDFLAGS};
my $current_kernel = do { use POSIX qw(uname); my ($__sys, $__node, $__rel, $__ver, $__mach) = POSIX::uname(); my @__parts; push @__parts, $__rel; join(" ", @__parts) . "\n"; };
my $current_os = do { use POSIX qw(uname); my ($__sys, $__node, $__rel, $__ver, $__mach) = POSIX::uname(); my @__parts; push @__parts, $__sys; join(" ", @__parts) . "\n"; };
my $running_distribution = do {
    my ($in_148, $out_148);
...
my $dkms_tree = "/var/lib/dkms";
my $source_tree = "/usr/src";
my $install_tree = "/lib/modules";
my $tmp_location = (defined ($ENV{TMPDIR} // q{}) && ($ENV{TMPDIR} // q{}) ne q{} ? ($ENV{TMPDIR} // q{}) : '/tmp');
my $verbose = "";
my $symlink_modules = "";
$tmpfile = do {
    my ($in_149, $out_149);
...
my $addon_modules_dir = ${ADDON_MODULES_DIR};
...
my $symlink_module;
my $directive_array[${#directive_array[@]}];
...
# Builtin command 'exec' not implemented
      ...
      or die "Close failed: $OS_ERROR\n";
        ...
        if ($ENV{1} =~ /$action_re/msx) {
            if (($action ne q{})) {
                                die(ret => 4, "$\"Cannot specify more than one action.\"");
                $CHILD_ERROR = 0;
            } else {
                $CHILD_ERROR = 1;
            }
...
if ((($binaries_only ne q{}) && ($source_only ne q{}))) {
    die(ret => 8, "$\" You have specified both --binaries-only and --source-only.\"", "$\"You cannot do this.\"");
}
if (eval { int(scalar(@kernelver) != scalar(@arch) &&     scalar(@arch) > 1) } // "") {
    die(ret => 1, "$\" If more than one arch is specified on the command line, then there\"", "$\"must be an equal number of kernel versions also specified (1:1 relationship).\"");
}
if ((($kernelver ne q{}) && ($all ne q{}))) {
    die(ret => 2, "$\" You cannot specify a kernel version and also specify\"", "$\"--all on the command line.\"");
}
if ((($arch ne q{}) && ($all ne q{}))) {
    die(ret => 3, "$\" You cannot specify an arch and also specify\"", "$\"--all on the command line.\"");
}
...
check_root();
    ...
    if (($(id -u) ne 0 && $force eq true)) {
        die(ret => 1, "$\"You must be root to use this command with the --force option.\"");
    }
        if (do {
load_tarball();
        $CHILD_ERROR == 0
